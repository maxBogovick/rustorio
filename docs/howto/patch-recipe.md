...
править, не переписывая. Для этого нужен jar-мод: правки выполняет код, а не данные. Как завести
jar-мод (точка входа, service-файл, сборка) — [code-mod.md](code-mod.md); здесь только сама правка.
