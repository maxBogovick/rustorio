...
public final class GfxConfig {
    ...
    /**
     * Сколько секунд держится один кадр «бегущей дорожки» ленты.
     *
     * <p>Это ЧИСТО визуальная скорость: она не связана с тем, как быстро лента реально
     * передаёт предметы. Когда у ленты появится дробная позиция предмета, анимацию можно
     * будет привязать к ней — пока дорожка просто бежит с постоянным темпом.
     */
    public static final float BELT_FRAME_SECONDS = 0.12f;
