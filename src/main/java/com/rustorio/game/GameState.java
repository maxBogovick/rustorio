 ...
 *
 * <p>{@code GameState} — «мешок состояния», живущий между кадрами: мир, пауза,
 * клетка под курсором. Ввод и отрисовка работают с этим объектом через методы,
 * а не лезут в чужие поля напрямую (инкапсуляция вместо «публичных полей»).
 *
 ...
 *
 * <p>Пока внутри тика ничего не происходит: мир пуст. Механизм времени уже
 * работает — по ходу курса в тело тика встанет шаг симуляции.
 */
public final class GameState {
    ...
    public void update(float deltaTime) {
        ...
        while (accumulator >= Config.TICK) {
            ...
            // Здесь появится шаг симуляции: буры копают, ленты везут, печи плавят.
        }
    }
     ...
     * секунду), а кадров рисуется шестьдесят. Когда по лентам поедут предметы,
     * без этого числа предмет дёргался бы скачками; с ним рендер показывает его
     * между прошлой и текущей позицией — движение плавное, хотя мир по-прежнему
     * думает целыми тиками.
     */
